...
module.exports = function(options) {
    ...
    return {
        ...
        down: function(fnStdout, fnStderr, fnExit){
            var execDockerCompose  = exec("docker-compose down --remove-orphans --rmi all -v", { cwd: cwd });
            execDockerCompose.stdout.on('data', function(data) { if(typeof fnStdout == "function") fnStdout(data.toString()); });
            execDockerCompose.stderr.on('data', function(data) { if(typeof fnStderr == "function") fnStderr(data.toString()); });
            execDockerCompose.on("exit", fnExit);
        },
        ...
        rm: function(serviceName, fnStdout, fnStderr, fnExit){
            let cmd = 'docker-compose rm -f';
            var execDockerCompose  = exec(cmd, { cwd: cwd });
            execDockerCompose.stdout.on('data', function(data) { 
              if(typeof fnStdout == "function") {
                fnStdout(data.toString());
              }
            });
            execDockerCompose.stderr.on('data', function(data) {
              if(typeof fnStderr == "function") {
                fnStderr(data.toString());
              }
            });
            if(fnExit) {
              execDockerCompose.on("exit", fnExit);
        ...
        up: function(fnStdout, fnStderr, fnExit){
            var execDockerCompose = exec("docker-compose up -d --force-recreate --build --quiet-pull --renew-anon-volumes", { cwd: cwd });
            execDockerCompose.stdout.on('data', function(data) { if(typeof fnStdout == "function") fnStdout(data.toString()); });
            execDockerCompose.stderr.on('data', function(data) { if(typeof fnStderr == "function") fnStderr(data.toString()); });
            execDockerCompose.on("exit", fnExit);
        },
